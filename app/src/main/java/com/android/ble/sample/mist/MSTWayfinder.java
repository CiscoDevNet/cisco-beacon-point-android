package com.android.ble.sample.mist;
...
/**
 * Created by Entappiainc on 5/9/16.
 */
public class MSTWayfinder {
    ...
    private ArrayList<MSTPoint> getPoints(Path path) {
        ...
        if (sEdgeName != null && pathArrayList != null) {
            for (int i = 0; i < pathArrayList.size(); i++) {
                        ...
                        && ( mstPath.getStartingEdgeName().equals(sEdgeName) || mstPath.getEndEdgeName().equals(sEdgeName)) )
                {
                    ...
                    {
                        double distance = distanceBetweenTwoPoints(xPos, yPos, mstPoint.getX() ,  mstPoint.getY() );

                        if (isFirstTime || distance < minDistance) {
            ...
            try {
                ...
                for (int i = 0; i < nodesJsonArray.length(); i++) {
                    ...
                    if (positionJsonObject != null) {
                        ...
                        double distance;
                        if(isActualData)
                            distance = distanceBetweenTwoPoints(point.getX()* mstMap.getPpm(), point.getY()* mstMap.getPpm(), Double.valueOf(xValue), Double.valueOf(yValue));
                        else
                            distance = distanceBetweenTwoPoints(point.getX(), point.getY(), Double.valueOf(xValue), Double.valueOf(yValue));
            ...
            try {
                for (int i = 0; i < edgesPointArrayList.size(); i++) {
                    ...
                    if (screenPoint != null) {
                        ...
                        double distance  = distanceBetweenTwoPoints(point.getX(), point.getY(), screenPoint.getX(), screenPoint.getY());
